...
- **Drag & drop** any `.ply` / `.splat` / `.ksplat` / `.spz` capture onto the page and it renders instantly — or a **`.zip` containing one** (Luma / Polycam exports open directly; a zip of capture *photos*, like the in-app camera burst, gets routed to the reconstruction guide instead, since training can't happen in a browser). Files are parsed entirely client-side; the privacy story is "your capture never leaves your machine."
