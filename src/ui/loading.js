...
export class LoadingOverlay {
  ...
  update(pct, label, phase) {
    if (Number.isFinite(pct)) this.fill.style.width = `${Math.min(100, Math.max(0, pct))}%`;
    this.phase.textContent = label ? `${phase} · ${label}` : phase;
    // Once splats are streaming in, let the user see them behind the overlay.
    if (phase === 'Processing' || (phase === 'Downloading' && pct > 30)) {
